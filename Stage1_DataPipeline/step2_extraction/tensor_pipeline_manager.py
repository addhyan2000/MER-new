...
class TensorPipelineManager:
    ...
    def run(self) -> int:
        ...
        if not tasks_queue:
            ...
            if len(valid_df) > 0:
                self.log.error(
                    "Step 2 produced no tensors for %d valid clip(s). "
                    "Re-run Preprocess with --force or check media paths.",
                    len(valid_df),
                )
                return 2
            return 0
        ...
        if success_count == 0:
            self.log.error("All %d clip extractions failed.", len(tasks_queue))
            return 1
        if success_count < len(tasks_queue):
            self.log.error(
                "Partial extraction failure: %d/%d clips succeeded.",
                success_count,
                len(tasks_queue),
            )
            return 1
