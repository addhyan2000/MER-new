...
class MerTestGuiApp(tk.Tk):
    ...
    def _real_data_ready(self) -> bool:
        ...
        if not excel.is_file() or not media.is_dir():
            return False
        mode = self.media_mode_var.get()
        if mode == "images":
            return any(media.rglob("sub*/*/*.jpg")) or any(media.rglob("sub*/*.jpg"))
    ...
    def _verify_step_output(self, header: str) -> tuple[int, str]:
        ...
        if "Step 2" in header:
            ...
            actual = len(list(tensor_dir.glob("*.npy"))) if tensor_dir.is_dir() else 0
            if actual == 0:
                return (
                    2,
            ...
            if actual < expected:
                return (
                    2,
                    f"\nFAILED verification: only {actual}/{expected} tensors in {tensor_dir}.\n",
                )
            return 0, ""
    ...
    def run_preprocess(self) -> None:
        if not self._real_data_ready():
            messagebox.showerror(
                "Real-data paths",
                "Set Excel + media root.\n\n"
                "Media type 'images': Cropped folder (subXX/clip/*.jpg)\n"
                "Media type 'avi': folder with subXX/*.avi\n\n"
                "Smoke tests do not need these paths.",
            )
            return
